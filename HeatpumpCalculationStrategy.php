...
class HeatpumpCalculationStrategy implements CalculationStrategy {
    ...
    public function calculatePrice($pool, $product){
        ...
        //=IF(E$13="binnenbad";$V$32/($AE$90*AH6/4);$V$7/($AE$90*AH6/4))
        /* Dit is de berekening zoals ze in de excel-sheet gebeurd. Een alternatieve berekening die nog getest moet worden, maar mogelijks een juister resultaat zal geven is lager te vinden. */
        /*$kw_januari;
        If (binnenbad) {
            $kw_januari = $kw_temp_hoog_binnen / ($gem_cop[2] * $kw_array / 4);
        } Else {
            $kw_januari = $kw_temp_hoog[1] / ($gem_cop[2] * $kw_array / 4);
        }
        $kw_februari;
        If (binnenbad) {
            $kw_februari = $kw_temp_hoog_binnen / ($gem_cop[2] * $kw_array / 4);
        } Else {
            $kw_februari = $kw_temp_hoog[2] / ($gem_cop[3] * $kw_array / 4);
        }
        $kw_maart;
        If (binnenbad) {
            $kw_maart = $kw_temp_hoog_binnen / ($gem_cop[2] * $kw_array / 4);
        } Else {
            $kw_maart = $kw_temp_hoog[3] / ($gem_cop[4] * $kw_array / 4);
        }
        $kw_april;
        If (binnenbad) {
            $kw_april = $kw_temp_hoog_binnen / ($gem_cop[2] * $kw_array / 4);
        } Else {
            $kw_april = $kw_temp_hoog[4] / ($gem_cop[5] * $kw_array / 4);
        }
        $kw_mei;
        If (binnenbad) {
            $kw_mei = $kw_temp_hoog_binnen / ($gem_cop[2] * $kw_array / 4);
        } Else {
            $kw_mei = $kw_temp_hoog[5] / ($gem_cop[6] * $kw_array / 4);
        }*/

        //=IF(E$13="binnenbad";$V$32/($AE$90*AH6/4);$V$7/($AE$90*AH6/4))
        /* Dit is een alternatieve berekening die nog getest moet worden, maar mogelijks een juister resultaat zal geven. De aanpassingen zijn echter minimaal. */
        /*$kw_januari;
        If (binnenbad) {
            $kw_januari = $kw_temp_hoog_binnen / ($gem_cop[1] * $kw_array / 4);
        } Else {
            $kw_januari = $kw_temp_hoog[1] / ($gem_cop[1] * $kw_array / 4);
        }
        $kw_februari;
        If (binnenbad) {
            $kw_februari = $kw_temp_hoog_binnen / ($gem_cop[1] * $kw_array / 4);
        } Else {
            $kw_februari = $kw_temp_hoog[2] / ($gem_cop[2] * $kw_array / 4);
        }
        $kw_maart;
        If (binnenbad) {
            $kw_maart = $kw_temp_hoog_binnen / ($gem_cop[1] * $kw_array / 4);
        } Else {
            $kw_maart = $kw_temp_hoog[3] / ($gem_cop[3] * $kw_array / 4);
        }
        $kw_april;
        If (binnenbad) {
            $kw_april = $kw_temp_hoog_binnen / ($gem_cop[1] * $kw_array / 4);
        } Else {
            $kw_april = $kw_temp_hoog[4] / ($gem_cop[4] * $kw_array / 4);
        }
        $kw_mei;
        If (binnenbad) {
            $kw_mei = $kw_temp_hoog_binnen / ($gem_cop[1] * $kw_array / 4);
        } Else {
            $kw_mei = $kw_temp_hoog[5] / ($gem_cop[5] * $kw_array / 4);
        }*/
        ...
        If ($product->getPeriod() == 1) { // 1 = Januari - December;
            ...
            If ($pool->getCovering() == "binnenbad") {
                ...
                $kw_oud = $KW_TEMP_HOOG[4] / ($GEM_COP[5] * 7.5) / $factor;
            }
        ...
        If ($product->getPeriod() == 1) { // 1 = Januari - December;
            ...
            If ($pool->getCovering() == "binnenbad") {
                ...
                $kw_nieuw = $KW_TEMP_HOOG[4] / ($GEM_COP[4] * 7.5) / $factor;
            }
        } Else { //Mag nooit gebeuren
            //Geef uw zwemperiode op.
        }
        ...
        $array["kw_oud"] = $kw_oud;
        $array["kw_nieuw"] = $kw_nieuw;
        
         $array["kw_oud_zomerpomp"] = Self::$ZOMERPOMPEN[round($kw_oud) - 1];
         $array["kw_nieuw_zomerpomp"] = Self::$ZOMERPOMPEN[round($kw_nieuw) - 1];
         $array["kw_oud_zomerpomp_alternatief"] = Self::$ZOMERPOMPEN_ALTERNATIEF[round($kw_oud) - 1];
         $array["kw_nieuw_zomerpomp_alternatief"] = Self::$ZOMERPOMPEN_ALTERNATIEF[round($kw_nieuw) - 1];
         $array["kw_oud_allseasonspomp"] = Self::$ALLSEASONSPOMPEN[round($kw_oud) - 1];
         $array["kw_nieuw_allseasonspomp"] = Self::$ALLSEASONSPOMPEN[round($kw_nieuw) - 1];
         $array["kw_oud_allseasonspomp_alternatief"] = Self::$ALLSEASONSPOMPEN_ALTERNATIEF[round($kw_oud) - 1];
         $array["kw_nieuw_allseasonspomp_alternatief"] = Self::$ALLSEASONSPOMPEN_ALTERNATIEF[round($kw_nieuw) - 1];
    ...
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2010", "Duratech Plus 7 kW zwembad én jacuzzi", 2250),
    array("ZVWX2020", "Duratech Plus 10 kW zwembad én jacuzzi", 3445),
    array("ZVWX2020", "Duratech Plus 10 kW zwembad én jacuzzi", 3445),
    array("ZVWX2020", "Duratech Plus 10 kW zwembad én jacuzzi", 3445),
    array("ZVWX2030", "Duratech Plus 14.3 kW zwembad én jacuzzi", 4250),
    array("ZVWX2030", "Duratech Plus 14.3 kW zwembad én jacuzzi", 4250),
    array("ZVWX2030", "Duratech Plus 14.3 kW zwembad én jacuzzi", 4250),
    array("ZVWX2030", "Duratech Plus 14.3 kW zwembad én jacuzzi", 4250),
    array("ZVWX2040", "Duratech Plus 19.6 kW zwembad én jacuzzi", 5875),
